...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (autoDetectBtn) {
        autoDetectBtn.addEventListener('click', function () {
            if (navigator.geolocation) {
                autoDetectBtn.textContent = '<i class="fa-solid fa-location-dot"></i> Detecting...';
                navigator.geolocation.getCurrentPosition(
                    function (position) {
                        console.log('Location detected:', position.coords);
                        autoDetectBtn.textContent = '<i class="fa-solid fa-check"></i> Location Detected';
                        setTimeout(() => {
                            autoDetectBtn.textContent = '<i class="fa-solid fa-location-dot"></i> Auto-Detect Current Location';
                        }, 2000);
                    },
                    function (error) {
    ...
    // Populate user info from sessionStorage if available
    ...
    if (currentUserStr) {
        try {
            ...
            if (currentUser.phone) {
                ...
                if (mobileNumberEl) mobileNumberEl.innerHTML = `${currentUser.phone} <span class="text-green-600"><i class="fa-solid fa-check"></i></span>`;
            }
        } catch (e) {
            console.error('Error reading currentUser in farmer profile:', e);
        }
    }
